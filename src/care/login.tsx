...
import { ArrowLeft, HeartPulse, Mail, MessageCircle, ShieldCheck, Stethoscope, Pill } from "lucide-react";
...
export function Login() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); void send(sent ? "verify" : "send"); }
  return <main className={`${styles.page} care-login`} dir="rtl" lang="ar">
    ...
    <div className={styles.layout}>
      <section className={styles.formPanel} aria-labelledby="login-title">
        ...
        <fieldset className={styles.channels} disabled={busy || sent}><legend>{c("login.choose")}</legend>{(["email", "whatsapp"] as const).map(value => <label key={value} className={channel === value ? styles.selected : ""}><input type="radio" name="channel" checked={channel === value} onChange={() => { setChannel(value); setIdentifier(""); setMessage(""); }} />{value === "email" ? <Mail size={19} aria-hidden="true" /> : <MessageCircle size={19} aria-hidden="true" />}{c(`login.${value}`)}</label>)}</fieldset>
        <form onSubmit={submit} className={styles.form}>
          <label htmlFor="identifier">{c(`login.${channel}`)}</label><input id="identifier" type={channel === "email" ? "email" : "tel"} autoComplete={channel === "email" ? "email" : "tel"} dir="ltr" required disabled={sent || busy} maxLength={254} value={identifier} placeholder={c(channel === "email" ? "login.emailPlaceholder" : "login.phonePlaceholder")} onChange={event => setIdentifier(event.target.value)} aria-describedby="login-hint" />
          <p id="login-hint" className={styles.hint}>{c(channel === "email" ? "login.emailHint" : "login.phoneHint")}</p>
          {sent && <><label htmlFor="otp">{c("login.code")}</label><input id="otp" inputMode="numeric" autoComplete="one-time-code" required minLength={6} maxLength={10} dir="ltr" value={token} onChange={event => setToken(event.target.value)} autoFocus /><p className={styles.hint}>{c("login.codeHint")}</p></>}
          <button className={styles.submit} disabled={busy} type="submit">{c(busy ? "common.loading" : sent ? "login.verify" : "login.send")} <ArrowLeft size={20} aria-hidden="true" /></button>
          {sent && <div className="care-inline"><button type="button" className="care-text-button" disabled={busy} onClick={() => { setSent(false); setToken(""); setMessage(""); }}>{c("login.change")}</button><button type="button" className="care-text-button" disabled={busy || cooldown > 0} onClick={() => void send("send")}>{c("login.resend")}{cooldown > 0 ? ` (${cooldown})` : ""}</button></div>}
